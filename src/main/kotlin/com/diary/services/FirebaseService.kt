...
object FirebaseService {
     ...
     *  3. Application Default Credentials — automatic on Cloud Run when Workload Identity is configured
     */
    ...
    suspend fun getUserById(userId: String): User? = withContext(Dispatchers.IO) {
        val doc = db.collection("users").document(userId).get().get()
        doc.toUser()
    }
    ...
    suspend fun createEntry(entry: DiaryEntry): DiaryEntry = withContext(Dispatchers.IO) {
        val docRef = db.collection("diaries").document(entry.id)
        docRef.set(entry.toMap()).get()
        entry
    }

    suspend fun getEntryById(entryId: String, userId: String): DiaryEntry? = withContext(Dispatchers.IO) {
        val doc = db.collection("diaries").document(entryId).get().get()
        val entry = doc.toDiaryEntry()
        if (entry?.userId == userId) entry else null
    }
